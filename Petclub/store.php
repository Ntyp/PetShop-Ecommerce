<?php
    ...
    $show = "SELECT * FROM stock_detail ORDER BY `stock_detail`.`Code` DESC";
    $resultshow = mysqli_query($conn,$show);
    // ลบสินค้า
    if($act == 'remove') {
        $Code = $_GET['Code'];
        $Size = $_GET['Size'];
        $remove = "DELETE FROM stock_detail WHERE Code = '$Code' && Size = '$Size'";
        $resultremove = mysqli_query($conn,$remove);
        echo '<script>alert("ลบสินค้าสำเร็จ");location.href="store.php";</script>';
    }
     ...
     if(isset($_POST['btnsubmit1'])) {
        ...
        $Choice = $_POST['Choice'];
        if($Choice == 'Plusitem') {
            $update = "UPDATE stock_detail SET Quatity = Quatity+'$Quatity' WHERE Code = '$Code' && Size = '$Size'";
            $resultupdate = mysqli_query($conn,$update);
            echo '<script>alert("อัปเดตสินค้าสำเร็จ");location.href="store.php";</script>';
        }
        else if($Choice == 'Minusitem') {
            $update = "UPDATE stock_detail SET Quatity = Quatity-'$Quatity' WHERE Code = '$Code' && Size = '$Size'";
            $resultupdate = mysqli_query($conn,$update);
            echo '<script>alert("อัปเดตสินค้าสำเร็จ");location.href="store.php";</script>';
        }
        ...
        <div class="container">
            <div class="row mt-5">
                        ...
                        <div class="row">
                            ...
                            <div class="col">
                                <select class="form-control" name="Choice" id="Choice">
                                    <option value="Plusitem">เพิ่มสินค้า</option>
                                    <option value="Minusitem">ลดสินค้า</option>
                                </select>
                            </div>
                            <div class="col">
                                <input class="btn btn-success" name="btnsubmit1" id="btnsubmit1" type="submit" value="ยืนยัน">
                            </div>
                        </div>
                        <!-- <div class="col">
                ...
                <table class="table table-bordered mt-3">
                    ...
                    ?>
                    <tr>
                        <td width='90'><?php echo $row['Code'] ?></td>
                        <td width='150'align="left"><?php echo $row['Name'] ?></td>
                        <td width='70' align="right"><?php echo $row['Price'] ?></td>
                        <td width='70' align="right"><?php echo $row['Quatity'] ?></td>
                        <td width='150'><?php echo $row['Size'] ?></td>
                        <td width='90' align="left"><a style="color:red;text-decoration:none"  href='store.php?Code=<?php echo $row['Code']; ?>&Size=<?php echo $row['Size']; ?>&act=remove'>ลบสินค้า</a></td>
                    </tr>
                    <?php 
                        }
                    ?>
